...
import Footer from "../components/Footer"; // Imported the brand footer here
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-between">
        ...
        <div className="max-w-5xl mx-auto -mt-10 md:-mt-20 relative z-10 bg-white rounded-t-3xl md:rounded-xl shadow-2xl p-5 md:p-8 md:mx-4 mx-0 border border-gray-100">
          ...
          <div className="flex gap-6 mb-6 md:mb-8">
            <label className="flex items-center gap-2 cursor-pointer">
              <input 
                type="radio" name="tripType" value="return" checked={tripType === "return"}
                onChange={() => setTripType("return")} className="w-5 h-5 accent-[#f5482b]"
              />
              <span className="font-bold text-black text-base md:text-lg">Return</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input 
                type="radio" name="tripType" value="one-way" checked={tripType === "one-way"}
                onChange={() => {
                  setTripType("one-way");
                  setReturnDate(""); 
                }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <AirportAutocomplete label="From" placeholder="Type city or code" value={fromAirport} onChange={setFromAirport} excludeCode={toAirport?.code} />
            <AirportAutocomplete label="To" placeholder="Type city or code" value={toAirport} onChange={setToAirport} excludeCode={fromAirport?.code} />

            <div className="flex flex-col">
              <label className="text-xs md:text-sm font-bold text-gray-500 mb-1 md:mb-2 uppercase">Departure</label>
              <input 
                type="date" min={tomorrowStr} value={departureDate}
                onChange={(e) => {
            ...
            <div className="flex flex-col">
              ...
              <input 
                type="date" min={departureDate || tomorrowStr} value={returnDate}
                onChange={(e) => setReturnDate(e.target.value)} disabled={tripType === "one-way"} 
      ...
      {/* 📥 ATTACHED THE GLOBAL FOOTER ELEMENT HERE */}
      <Footer />
    </div>
  );
}