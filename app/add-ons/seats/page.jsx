...
// Mocking some randomly taken seats so the plane looks real
const OCCUPIED_SEATS = ["1A", "1C", "2F", "2J", "4B", "4C", "4D", "5H", "5J", "8A", "8B", "10D", "10E", "10F", "12J", "14A", "15G"];

// Helper to determine what type of seat this is based on Row and Letter
const getSeatDetails = (row, letter) => {
  if (row <= 3) return SEAT_PRICING.business;
  if (row <= 5) return SEAT_PRICING.legroom;
  
  // Standard Economy Rows (6+)
...
function SeatsContent() {
  ...
  // seatCart stores selections: { "flyer-1": { code: "12A", price: 15, type: "Window" } }
  ...
  const handleSeatClick = (code, seatDetails) => {
    ...
    if (alreadyAssignedTo) {
      // If the ACTIVE passenger clicks their own seat, unselect it
    ...
    // Auto-advance to the next passenger who doesn't have a seat yet
  ...
  const handleNext = () => {
    // Ensure everyone has a seat if they want to proceed (optional, but good UX)
    ...
    // Moving to final checkout folder (we will build this next)
    window.location.href = `/checkout?${searchParams.toString()}`;
  };

  // Reusable Seat Button Component
  const SeatButton = ({ row, letter }) => {
    ...
    return (
      <div 
        onClick={() => handleSeatClick(code, details)} 
        className={buttonClass}
        title={`${code} - ${details.label} (+$${details.price})`}
      >
        <span className="text-xs font-black">{code}</span>
        {assignedPassengerId && (
          <span className="text-[10px] font-bold">P{assignedPassengerId.split('-')[1]}</span>
        )}
      </div>
    );
  };
    ...
    <div className="max-w-6xl mx-auto py-8 px-4 flex flex-col lg:flex-row gap-8">
      ...
      <div className="w-full lg:w-2/3">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-wrap gap-4 md:gap-8 mb-8 text-xs font-bold text-gray-700">
          ...
          <div className="flex items-center gap-2"><div className="w-5 h-5 rounded bg-[#f5482b] border-2 border-[#d83c20]"></div> Selected</div>
          <div className="flex items-center gap-2"><div className="w-5 h-5 rounded bg-gray-200 border-2 border-gray-300"></div> Occupied</div>
        </div>
        ...
        <div className="bg-gray-100 rounded-t-[100px] border-4 border-gray-300 p-4 md:p-8 overflow-x-auto">
          <div className="min-w-[600px] flex flex-col gap-6 mx-auto">
            ...
            {/* BUSINESS CLASS (Rows 1-3) 2-2-2 Layout */}
            <div className="flex flex-col gap-4 pb-8 border-b-2 border-dashed border-gray-300 relative">
              <div className="absolute -left-2 top-0 bottom-0 flex items-center">
                <span className="origin-center -rotate-90 text-amber-500 font-black tracking-widest uppercase text-xs">Business Class</span>
              </div>
              {[1, 2, 3].map(row => (
                <div key={`biz-${row}`} className="flex justify-center items-center gap-6">
                  <div className="flex gap-2">
                    <SeatButton row={row} letter="A" />
                    <SeatButton row={row} letter="C" />
                  </div>
                  <div className="w-8 font-black text-gray-400 text-center">{row}</div>
                  <div className="flex gap-2">
                    <SeatButton row={row} letter="D" />
                    <SeatButton row={row} letter="F" />
                  </div>
                  <div className="w-8"></div>
                  <div className="flex gap-2">
                    <SeatButton row={row} letter="G" />
                    <SeatButton row={row} letter="J" />
                  </div>
                </div>
              ))}
            </div>

            {/* ECONOMY EXTRA LEGROOM (Rows 4-5) 3-3-3 Layout */}
            <div className="flex flex-col gap-3 pb-8 border-b-2 border-dashed border-gray-300 relative mt-4">
               <div className="absolute -left-2 top-0 bottom-0 flex items-center">
                <span className="origin-center -rotate-90 text-purple-500 font-black tracking-widest uppercase text-xs">Extra Legroom</span>
              </div>
              {[4, 5].map(row => (
                <div key={`leg-${row}`} className="flex justify-center items-center gap-4">
                  <div className="flex gap-1">
                    <SeatButton row={row} letter="A" />
                    <SeatButton row={row} letter="B" />
                    <SeatButton row={row} letter="C" />
                  </div>
                  <div className="w-8 font-black text-gray-400 text-center">{row}</div>
                  <div className="flex gap-1">
                    <SeatButton row={row} letter="D" />
                    <SeatButton row={row} letter="E" />
                    <SeatButton row={row} letter="F" />
                  </div>
                  <div className="w-8"></div>
                  <div className="flex gap-1">
                    <SeatButton row={row} letter="G" />
                    <SeatButton row={row} letter="H" />
                    <SeatButton row={row} letter="J" />
                  </div>
                </div>
              ))}
            ...
            {/* ECONOMY STANDARD (Rows 6-15) 3-3-3 Layout */}
            <div className="flex flex-col gap-3 relative mt-4">
              <div className="absolute -left-2 top-10 bottom-0 flex items-center">
                <span className="origin-center -rotate-90 text-blue-500 font-black tracking-widest uppercase text-xs">Standard Economy</span>
              </div>
            ...
            {passengers.map(p => {
              ...
              return (
                <div 
                  ...
                  className={`p-3 rounded-lg border-2 flex justify-between items-center cursor-pointer transition-all ${isActive ? 'border-[#f5482b] bg-red-50' : 'border-gray-100 hover:border-gray-300'}`}
                >
                  <div>
                    ...
                    <div className="text-xs text-gray-500 font-bold ml-4">
                      {seat ? `${seat.type} (${seat.code})` : "Selecting..."}
                    </div>
                  </div>
